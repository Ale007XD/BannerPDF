...
| | |
|---|---|
| Готовность | **95%** — сайт запущен, CI зелёный |
| URL | `https://bannerbot.ru:8444` |
| Что готово | Весь бэкенд (P0 закрыты), фронтенд (динамический рендер из шаблонов, sticky-превью на мобиле и десктопе, кастомный размер, защита от совпадения цветов), схема БД, Nginx, Dockerfile, docker-compose, GitHub Actions, автодеплой, SSL |
...
│   │   ├── style.css             # Mobile-first + sticky-превью на мобиле + desktop двухколонка (≥820px)
│   │   └── app.js                # loadTemplates(), кастомный размер, защита цветов, дебаунс 500мс, поллинг 2.5с
│   │
│   └── nginx/
...
2. Пересобирает контейнер `api` без кэша только если изменился `Dockerfile` или `requirements.txt`, иначе — с кэшем (~15 сек вместо ~2 мин)
3. Поднимает `docker compose up -d --force-recreate --remove-orphans`
4. Чистит висячие образы `docker image prune -f`
5. Проверяет health: `docker exec bannerprint_api python -c "import urllib.request, sys; r=urllib.request.urlopen('http://localhost:8000/api/health'); sys.exit(0 if b'ok' in r.read() else 1)"`
...
### Пример: создание заказа с типовым размером
...
### Пример: создание заказа с кастомным размером
...
| `test_preview_api.py` | HTTP: POST /api/preview, все size_key, кастомный размер, цвета |
...
- [x] Автодеплой через GitHub Actions (умная пересборка + health check)
- [x] SSL-сертификат (bannerbot.ru, истекает 2026-06-19)
- [x] Фронтенд: динамический рендер размеров/цветов/шрифтов из `/api/templates`
- [x] Фронтенд: sticky-превью на мобиле (под шапкой) и десктопе (двухколонка ≥820px)
- [x] Фронтенд: кастомный размер 100–3000 мм (ширина × высота)
- [x] Фронтенд: защита от совпадения цветов фона и текста
- [x] Бэкенд: `renderer.py` и `sanitizer.py` поддерживают `width_mm`/`height_mm` наравне с `size_key`
- [x] `config.py`: `MIN_DIMENSION` 500→100, добавлен `2x2` в `BANNER_SIZES`
...
- [ ] `test_sanitizer.py`: добавить тесты для кастомного размера (width_mm/height_mm, граничные значения)
- [ ] `test_preview_api.py`: добавить тест POST /api/preview с width_mm/height_mm
