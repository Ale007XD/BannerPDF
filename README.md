...
| | |
|---|---|
| Готовность | **98%** — сайт запущен, принимает заказы |
| URL | `https://bannerbot.ru:8444` |
| Что готово | Весь бэкенд (P0 закрыты), фронтенд (превью, кастомный размер, bottom sheet на мобиле, шрифты в кнопках), FSM заказов, ручной флоу оплаты (СБП + adminка), `POST /api/admin/force_token`, мобильная админка `/admin/` |
...
│   │   ├── style.css             # Mobile-first, FAB + bottom sheet на мобиле, двухколонка ≥820px
