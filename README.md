...
| | |
|---|---|
| Готовность | **95%** — сайт запущен, принимает заказы через ЮКасса |
| URL | `https://bannerbot.ru` |
| Что готово | Весь бэкенд (P0 закрыты), фронтенд (превью, кастомный размер, шрифты в кнопках, ЮКасса-виджет), FSM заказов, E2E флоу оплаты (ЮКасса → webhook → PDF), TG-уведомления (`@BannerBotInfo_bot`), inline-кнопка «Выдать PDF» в TG, корп. API с тарифом Trial, adminка |
| Что осталось | Отладка TG-кнопки «Выдать PDF» · боевые ключи ЮКасса · реферальные команды бота · тесты корп. API |
...
```
  ...
  ЮКасса webhook ──► POST /api/payment/callback
                         ...
                         └─ editMessageText в TG (@BannerBotInfo_bot)
                         └─ Клиент получает PDF автоматически (поллинг)

  TG inline-кнопка ──► POST /api/tg/callback → do_force_token()
...
`do_force_token()` вынесена из HTTP-эндпоинта — вызывается как из adminки, так и из `tg_webhook.py` без обхода `Depends(require_admin)`.
...
ЮКасса embedded-виджет — работает с момента запуска.
...
**Ручная выдача (force_token) через curl:**

```bash
curl -X POST https://bannerbot.ru/api/admin/force_token/<ORDER_ID> \
...
### P1 — кнопка «Выдать PDF» в TG

- [ ] Разобраться почему кнопка «Выдать PDF» в TG залипает без реакции
- [ ] Проверить `tg_webhook.py` + регистрацию webhook на порту 88

### P1 — боевые ключи ЮКасса

- [ ] Получить боевые `YOOKASSA_SHOP_ID` + `YOOKASSA_SECRET_KEY`
- [ ] Обновить `.env` → `docker compose restart api`
- [ ] Подтвердить webhook URL в ЛК ЮКасса: `https://bannerbot.ru/api/payment/callback`
