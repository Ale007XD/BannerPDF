...
payment_selfwork.py
~~~~~~~~~~~~~~~~~~~
Запасной платёжный провайдер: Сам.Эквайринг (selfwork.ru).

Активный провайдер — ЮКасса (services/payment.py).
Чтобы переключиться на Selfwork:
  1. В routers/order.py: from ..services.payment_selfwork import create_payment
  2. В main.py: from .routers import payment_selfwork as payment
  3. Задать env: SELFWORK_SHOP_ID, SELFWORK_API_KEY
