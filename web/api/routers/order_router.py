...
Содержит:
  ...
  - POST /api/order — создание заказа + создание платежа в ЮKassa
...
async def create_order(req: OrderRequest):
    """
    Создаёт заказ и возвращает токен для инициализации виджета ЮKassa.
    ...
    4. Создаёт платёж в ЮKassa через POST /v3/payments
    5. Сохраняет yookassa_payment_id в web_orders
    6. Возвращает {order_id, confirmation_token}
    """
    config = {
    ...
    # Подготавливаем платёж в ЮKassa (HTTP-запрос к их API)
    try:
    ...
    with get_db() as conn:
        conn.execute(
            ...
            (payment["yookassa_payment_id"], order_id),
        )
